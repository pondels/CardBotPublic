...
class Uno():
    ...
    def draw_card(self, username):
        ...
        if player['uno']: gameCollection.update_one({'_id': player['_id']}, {"$set": {'uno': False}})
    ...
    def check_valid_card(self, username, n):
        # Returns
        #   Bool
        #   Card as dict object
        #   List of Players
        ...
        for player in players:
            if len(player['hand']) == 1 and not player['uno']:
                if player['_id'] == username:
                    ...
                    cards = []
                    for _ in range(2):
                        card, _ = self.draw_card(player['_id'])
                        cards.append(card)
                    return ('caught', cards, player['_id'])
        return ('No valid player found to call uno!')